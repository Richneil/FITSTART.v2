  ...
  return (
    <div className="mt-3 pt-3 border-t border-surface-200/60 dark:border-surface-700/60">
      <button
        ...
        className="flex items-center justify-between w-full text-xs font-bold text-brand-700 dark:text-brand-300 hover:text-brand-800 dark:hover:text-brand-200 transition-colors py-1 group"
      >
        <span className="flex items-center gap-1.5">
          <Calculator className="w-3.5 h-3.5 text-brand-600 dark:text-brand-400" />
          {isOpen ? 'Hide Relevance Calculation Math' : 'Show the Math (Weight Breakdown)'}
        </span>
        <span className="text-[10px] bg-brand-100 dark:bg-brand-900/60 text-brand-800 dark:text-brand-200 px-2 py-0.5 rounded-md font-bold group-hover:bg-brand-200 transition-colors">
      ...
      {isOpen && (
        <div className="mt-3 p-3.5 bg-surface-50 dark:bg-surface-800/80 rounded-2xl border border-surface-200 dark:border-surface-700 animate-slide-up text-left">
          <div className="flex justify-between items-center pb-2 mb-2.5 border-b border-surface-200 dark:border-surface-700 text-[11px] font-extrabold text-surface-700 dark:text-surface-300">
            <span>Calculation Waterfall</span>
          ...
          <div className="space-y-2 text-xs">
            {metric.steps.map((step, idx) => (
              <div key={idx} className="flex items-start justify-between gap-3 text-surface-700 dark:text-surface-300">
                ...
                <span className="font-extrabold text-surface-900 dark:text-white text-xs shrink-0 mt-0.5 font-mono">
                  = {step.subtotal} pts
                </span>
              </div>
            ))}
          ...
          <div className="mt-3 pt-2.5 border-t border-surface-200 dark:border-surface-700 flex justify-between items-center text-xs font-black text-brand-900 dark:text-brand-200">
            <span>Final Relevance Score:</span>
            <span className="text-xs bg-brand-500 text-white px-2.5 py-0.5 rounded-lg shadow-sm">
