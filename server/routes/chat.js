  ...
  try {
    ...
    if (!apiKey) {
      ...
      if (q.includes('calculate') || q.includes('score') || q.includes('math') || q.includes('points')) {
        reply = `Based on your results, your priority scores are calculated using the FitStart decision engine. Each metric begins with a baseline relevance score and receives point deltas based on your declared goals, weekly training schedule, and nutrition habits.`;
